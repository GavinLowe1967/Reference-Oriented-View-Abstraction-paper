...
\framebox{Change to lock-based queue}
...
Figure~\ref{fig:lock-based-stack} gives a more interesting example,
representing a lock-based stack that uses a linked list internally;
pseudo-code is in Figure~\ref{fig:lock-based-stack-pseudocode}.  Here the
components come from two families: nodes that make up the linked list; and
threads that operate on the stack.  
